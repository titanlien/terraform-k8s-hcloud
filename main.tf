...
resource "null_resource" "ansible-master" {
  ...
  provisioner "local-exec" {
    command = "ssh-keyscan -H '${hcloud_server.master.*.ipv4_address}' >> ~/.ssh/known_hosts; ansible-playbook -e sshKey=${var.ssh_private_key} -i ./ansible/inventory --limit managers ./ansible/site.yaml --ask-vault-pass -u root"
  }
...
resource "null_resource" "ansible-node" {
  ...
  provisioner "local-exec" {
    command = "ssh-keyscan -H '${hcloud_server.node.*.ipv4_address}' >> ~/.ssh/known_hosts; ansible-playbook -e sshKey=${var.ssh_private_key} -i ./ansible/inventory --limit nodes ./ansible/site.yaml --ask-vault-pass -u root"
  }
